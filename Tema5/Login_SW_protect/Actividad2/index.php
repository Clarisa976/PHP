<?php
/*session_name("login_pro");
session_start();*/
require "src/funciones_ctes2.php";
...
if(isset($_POST["btnContAgregar"])){
    ...
    $error_cod = $_POST["cod"]=="";
    if(!$error_cod){
        $url = DIR_SERV2 . "/repetido/producto/cod/".urlencode($_POST["cod"]);
    ...
    if(!$error_nombre_corto){
        $url = DIR_SERV2 . "/repetido/producto/nombre_corto/".urlencode($_POST["nombre_corto"]);
        $respuesta = consumir_servicios_REST($url, "GET");
        $json_repetido = json_decode($respuesta, true);
        if (!$json_repetido)
            die("<p> Error consumiendo servicio web <strong>" . $url . "</strong></p></body></html>");
    ...
    if(!$error_form){
        ...
        $url = DIR_SERV2 . "/producto/insertar";
...
if (isset($_POST["btnDetalles"])) {
    $url = DIR_SERV2 . "/producto/" . $_POST["btnDetalles"] . "";
    $respuesta = consumir_servicios_REST($url, "GET");
...
if (isset($_POST["btnContBorrar"])) {
    $url = DIR_SERV2 . "/producto/borrar/" . $_POST["btnContBorrar"] . "";
